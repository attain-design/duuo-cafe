  ...
  return <section id="galerie" className="bg-white py-20">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {instagramPosts.map(post => <a key={post.id} href={post.link} target="_blank" rel="noopener noreferrer" className="instagram-image overflow-hidden rounded-lg">
              <img src={post.image} alt={post.alt} className="h-80 w-full object-cover transition-all duration-500" />
            </a>)}
        </div>

        <div className="mt-10 flex justify-center">
          <a href="https://www.instagram.com/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-full bg-cafe-main px-6 py-3 text-white transition-all duration-300 hover:bg-cafe-dark">
